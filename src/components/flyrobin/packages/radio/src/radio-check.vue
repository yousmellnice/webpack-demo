<template>
  <label
    class="fr-radio-check"
    :class="[
      border && radioSize ? 'fr-radio-check--' + radioSize : '',
  ...
  >
    <span class="fr-radio-check__input" :class="{ 'is-disabled': isDisabled, 'is-checked': model === label }">
      <span class="fr-radio-check__inner" :style="model === label ? activeInnerStyle : null"> </span>
      <input
        ref="radio"
        class="fr-radio-check__original"
        :value="label"
        type="radio"
        v-model="model"
        @focus="focus = true"
    ...
    <span class="fr-radio-check__label" :style="model === label ? activeStyle : null" @keydown.stop>
      <slot></slot>
